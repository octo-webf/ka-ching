...
app.controller("HomeCtrl",function ($scope,$rootScope,$http,$window)
{
  ...
  function(){$scope.error="Impossible de retrouver vos amis";});

  $http({url:"http://localhost:3000/api/account",method: "GET"}).then(
  function(response){$scope.balance=response.data.balance;});

  $scope.addFriend=function () {
  if ($scope.showModal == true) {
    $scope.showModal = false
  } else if ($scope.showModal==false){$scope.showModal = true;}
  $scope.friend={user:{
  ...
  $scope.transferMoney = function (friend) {
    ...
    $http({
      ...
      method: "PUT",
      data: {transfer: {recipient: friend, amount: friend.transferedAmount, date: new Date()}}}).then(
  ...
  $scope.transferHistory = function () {
    ...
    $http({url: "http://localhost:3000/api/transfers", method: "GET"}).then(
